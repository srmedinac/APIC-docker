...
if hasattr(os, 'add_dll_directory'):
    with os.add_dll_directory(r'C:\Program Files\openslide\bin'):
...
####
from scipy.ndimage import measurements, binary_fill_holes, label
...
def infer_step(patch_imgs):
    ...
    #patch_imgs_gpu = patch_imgs_gpu.to("cuda:0").type(torch.float32)  # to NCHW
...
def getSaturationMetric(image):
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    s=hsv[:,:,1]/255
    return sum(sum(s))/s.size

def get_nuc_type_id(type_name):
    for key in type_info.keys(): 
        if type_info[key][0]==type_name:
            return key

def get_img_from_json_coords(tileW,tileH,value):
    data = json.loads(value)
    im = np.zeros((tileW,tileH), 'uint8')
    for nuc in data:
        type_id=get_nuc_type_id(nuc['properties']['classification']['name'])
        coords=nuc['geometry']['coordinates']
        for c in coords:
            poly=np.array(c)
            rr, cc = polygon(poly[:,0], poly[:,1], im.shape)
            im[cc,rr] = int(type_id)+1
    return im

def processOneTile(tile):
    offset = 92//2
